...
class Block():
	...
	def __init__(self, id, ledger, prev_hash, difficulty, nonce=0):
		self.id = id
		self.ledger = ledger
...
Nonce: {4}
Hash: {5}
			""").format(self.id, self.ledger, self.prev_hash, self.difficulty,
	...
	def mine_valid_block_hash(self):
		...
		block_str = str(self.id) + json.dumps(self.ledger) + self.prev_hash
